...
const ApexDonutChart = () => {
  ...
  let nb = 0
  let j = 1
  for (let i = 0; i < state.data2.length; i++) {
    nb += state.data2[i].count
    if (j == 6) {
      j = 1
    }
  ...
  const options: ApexOptions = {
    stroke: { width: 0 },
    labels: state.data2.map((el: any) => el?.fullName) || [],
    colors: ColorArr,
    dataLabels: {
      enabled: true,
    ...
    <Card>
      <CardHeader
        ...
        action={
          <Box style={{ display: 'flex', gap: '0.5rem', width: '100%' }}>
            ...
            <Box style={{ width: '100%' }}>
              <CustomSelect setData={setmaterials} haseAll={false} data={customSelectChnatierMaterail} />
            </Box>
          </Box>
        }
      ...
      <CardContent>
        <ReactApexcharts
          ...
          height={400}
          options={options}
          series={state.data2.map((el: any) => el?.count) || []}
        />
      </CardContent>
    </Card>
  )
